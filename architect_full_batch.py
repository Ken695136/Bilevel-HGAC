...
class Architect():
    ...
    def unrolled_backward(self, tr_input1, tr_input2, val_input1, val_input2, xi, w_optim, labels, train_idx
                          , val_idx):
        ...
        self.virtual_step(tr_input1, tr_input2, xi, w_optim,labels,train_idx)
        ...
        hessian = self.compute_hessian(dw, tr_input1, tr_input2, labels, train_idx)        # 梯度近似后公式第二项
    ...
    def compute_hessian(self, dw, tr_input1, tr_input2, labels,train_idx):
        """
        求经过泰勒展开后的第二项的近似值
        ...
        logp = F.log_softmax(logits, 1)
        train_loss = F.nll_loss(logp[train_idx], labels[train_idx])     # L_trn(w+)
