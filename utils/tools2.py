...
def evaluate_results_nc(embeddings, labels, num_classes):
    ...
    macro_mean = [x for (x, y) in svm_macro_f1_list]
    micro_mean = [x for (x, y) in svm_micro_f1_list]
    return np.array(macro_mean), np.array(micro_mean), nmi_mean, nmi_std, ari_mean, ari_std
...
def select_top_k(adj,k):
    adj = adj.todense()
    s_a=np.argsort(adj[4019:11186,0:4019],axis=1)
    s_s = np.argsort(adj[11186:,0:4019],axis=1)
    news_a = s_a[:, 0:4019 - k]
    news_s = s_s[:, 0:4019 - k]
    for i in range(news_a.shape[0]):
        for j in range(news_a.shape[1]):
            adj[i+4019, news_a[i,j]] = 0
            adj[news_a[i,j],i+4019] = 0
    for i in range(news_s.shape[0]):
        for j in range(news_s.shape[1]):
            adj[i+11186, news_s[i,j]] = 0
            adj[news_s[i,j],i+11186] = 0
    adj[adj > 0] = 1
    return adj
